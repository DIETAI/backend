...
import { IDinnerPortionsGenerateArgs } from '../dinnerPortionGenerate';
...
export const cartesianDinners = (
  ...
  mealEstablishment: IDinnerPortionsGenerateArgs['mealEstablishment'],
  currentProcent: number,
  ...portions: DinnerPortionsMacro
) => {
  ...
  const helper = (arr: DinnerPortionsMacro[0], i: number) => {
...
type MacroForPortions = DinnerPortionsMacro[0];
...
const cartesianGroupTotalCount = (groups: MacroForPortions) => {
  ...
  const total_fiber_gram = roundMacro(
    countCartesianMacroTotal(groups, 'portionFiberGram')
  );

  const total_fiber_kcal = roundMacro(
    countCartesianMacroTotal(groups, 'portionFiberKcal')
  );

  const total_digestableCarbohydrates_gram = roundMacro(
    countCartesianMacroTotal(groups, 'portionDisgestibleCarbohydratesGram')
  );
  const total_digestableCarbohydrates_kcal = roundMacro(
    countCartesianMacroTotal(groups, 'portionDisgestibleCarbohydratesKcal')
  );
